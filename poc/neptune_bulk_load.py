...
def start_bulk_load(
    ...
    resp = requests.post(url, json=payload, timeout=60, verify=VERIFY_TLS)
...
def poll_bulk_load(neptune_endpoint: str, port: int, load_id: str, poll_seconds: int = 10) -> dict:
    ...
    while True:
        ...
        status = overall.get("status") or overall.get("overallStatus")  # some versions use 'status'
...
def main():
    ...
    IAM_ROLE_ARN = os.environ["IAM_ROLE_ARN"]           # role with S3 read perms and Neptune load perms
    ...
    LOCAL_EDGE_FILE = os.environ.get("LOCAL_EDGE_FILE", "edges.csv")
    # ------------------------
    ...
    #    bucket=S3_BUCKET,
    #    prefix=S3_PREFIX,
    #    local_files=[LOCAL_NODE_FILE, LOCAL_EDGE_FILE],
    #    region=AWS_REGION,
    #)
